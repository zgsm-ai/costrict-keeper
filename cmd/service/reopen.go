...
var reopenCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if rpcClient != nil && tryReopenTunnelViaRPC(rpcClient, serviceName) {
			// RPC 调用成功，直接返回
			return
		}
...
// tryReopenTunnelViaRPC 尝试通过 RPC 连接启动隧道
/**
 ...
 * @example
 * success := tryReopenTunnelViaRPC(rpcClient, "myapp", 8080)
 * if success {
 *     fmt.Println("Tunnel started via RPC")
 * }
 */
func tryReopenTunnelViaRPC(rpcClient rpc.HTTPClient, appName string) bool {
	// 尝试调用 costrict 的 RESTful API
	response, err := rpcClient.Post(fmt.Sprintf("/costrict/api/v1/services/%s/reopen", appName), nil)
	if err != nil {
